...
fn main() {
    ...
    let solvers = day_to_solvers.get(day.as_str()).expect(&format!("{day} not found"));
    solvers.iter().enumerate().for_each(|(index, solver)| {
        let input_index = index + 1;
        let path = format!("src/aoc2023/{day}/input{input_index}.txt");
        let input = fs::read_to_string(&path).expect(&format!("Something went wrong reading the file {path}"));
